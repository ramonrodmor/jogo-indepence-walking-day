...
Renderer::Renderer()
{
	...
	spriteStorage = new SpriteInfo[MaxBatchSize];
	storageIndex = 0;
...
{
	if (point->X() >= Game::viewport.left && point->X() < Game::viewport.right)
		if (point->Y() >= Game::viewport.top && point->Y() < Game::viewport.bottom)
			PlotPixel(int(point->X() - Game::viewport.left), int(point->Y() - Game::viewport.top), color);
}
...
int Renderer::ClipLine(int &x1, int &y1, int &x2, int &y2)
{
	...
	int min_clip_x = int(Game::viewport.left);
	int min_clip_y = int(Game::viewport.top);
	int max_clip_x = int(Game::viewport.right - 1);
	int max_clip_y = int(Game::viewport.bottom - 1);
...
{
	a1 -= int(Game::viewport.left);
	b1 -= int(Game::viewport.top);
	a2 -= int(Game::viewport.left);
	b2 -= int(Game::viewport.top);
...
void Renderer::Draw(Multi * mul, ulong color)
{
	for (auto i = mul->shapes.begin(); i != mul->shapes.end(); ++i)
		Draw(*i, color);
}
...
bool Renderer::Initialize(Window * window, Graphics * graphics) 
{
	...
	D3D11_SUBRESOURCE_DATA indiceData = { 0 };
	indiceData.pSysMem = &indices.front();

	if FAILED(graphics->Device()->CreateBuffer(&indexBufferDesc, &indiceData, &indexBuffer))
		return false;
	...
	pixelPlotSprite.color = Color(1, 1, 1, 1);
...
void Renderer::RenderBatch(ID3D11ShaderResourceView * texture, SpriteInfo ** sprites, uint count)
{
...
void Renderer::Render()
{
	...
	sort(spriteVector.begin(), spriteVector.end(), 
		[](SpriteInfo * a, SpriteInfo * b) -> bool 
...
void Renderer::Draw(SpriteInfo & sprite)
{
	if (storageIndex < MaxBatchSize)
	{
		spriteStorage[storageIndex] = sprite;
		spriteVector.push_back(&spriteStorage[storageIndex]);
		storageIndex++;
	}
}
